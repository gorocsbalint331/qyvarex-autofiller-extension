...
export function getOracleCloudStepState(options = {}) {
  ...
  if (regexMatch) {
    ...
    return {
      index: ORACLE_CLOUD_APPLY_SECTION_BASE_INDEX + element,
      pageNumber: element,
      title: sectionTitle,
      isApplySection: true,
...
export const OBSERVER_LIST = {
  ...
  myworkday: function(handle, hasClickedAutoFill) {
    ...
    return [startMyWorkdayObserver, () => {
      document.removeEventListener("click", onContinueClick, true), registerPaginationAdvanceCheck(siteKey, void 0), itemList.forEach(clearTimeout), itemList.length = 0, clearTimeout(retryTimeoutId), pollIntervalId && clearInterval(pollIntervalId), mutationObserver && mutationObserver.disconnect();
    }];
  },
