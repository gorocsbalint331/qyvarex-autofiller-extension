...
import * as u from "../../../core/xpath.js"
...
async function waitForRipplingResumeParsingComplete(e = {}) {
  let t = A();
  if (hasRipplingResumeParsingSucceeded() && !e.afterUpload) return await k(), true;
  let r = document.body, n = await observer.waitForCondition(() => S() || hasRipplingResumeParsingSucceeded(), { timeout: 1500, interval: 100, observeTarget: r });
  if (!n && !e.force && !E()) return true;
  let o2 = t, i2 = Date.now(), a2 = false, s2 = await observer.waitForCondition(() => {
    if (hasRipplingResumeParsingSucceeded()) return true;
    let e2 = A();
...
async function K(e, t, r) {
  let n = () => q(readRipplingPhoneCodeDisplayText(), r), o2 = Y(e);
  if (!o2) return false;
...
async function selectPhoneCountryCode(e, t = {}) {
  try {
    ...
    if (!o2) return false;
    let i2 = () => q(readRipplingPhoneCodeDisplayText(), r);
    if (i2()) return true;
...
async function uploadResume(e, t, r) {
  ...
  if (n.includes("rippling-ats")) {
    ...
    if (n2) {
      ...
      l2 && (await dom.uploadFiles(l2, await answerMethods.fetchPdfAsBlob(e), t, r, "Resume/CV"), await waitForRipplingResumeParsingComplete({ afterUpload: true }));
    }
  } else {
    let n2 = document.querySelector('label[data-testid="resume"]');
    if (n2) {
      let o2 = n2.querySelector('input[type="file"]');
      o2 && (await dom.uploadFiles(o2, await answerMethods.fetchPdfAsBlob(e), t, r, "Resume/CV"), await waitForRipplingResumeParsingComplete({ force: "ats.rippling.com" === window.location.hostname, afterUpload: true }));
    }
  }
}
function Q(e) {
...
function hasRipplingCoverLetterSlot() {
  let e = getRipplingCoverLetterUploadDom();
  return !!(e.label || e.container || e.dropzone || e.input);
}
async function waitForRipplingCoverLetterSlot() {
  return await observer.waitForCondition(() => hasRipplingCoverLetterSlot(), { timeout: 3e3, interval: 100, observeTarget: document.body });
}
async function el() {
  let e = getRipplingCoverLetterUploadDom();
  ...
  let r = await observer.waitForCondition(() => {
    let e2 = getRipplingCoverLetterUploadDom(), r2 = e2.hiddenInputs.find((e3) => !t.has(e3));
    return !!(e2.input || r2);
  }, { timeout: 1500, interval: 50, observeTarget: document.body });
  if (!r) return null;
  let n = getRipplingCoverLetterUploadDom(), o2 = n.hiddenInputs.find((e2) => !t.has(e2));
...
async function fillResolvedLocationInput(e, t) {
  ...
  if (!(r instanceof HTMLInputElement)) return fillInputTextField(r, n);
  try {
  ...
  let o2 = null, i2 = () => {
    ...
    return !!(o2 = findExactRipplingLocationOption(n, t2));
  };
