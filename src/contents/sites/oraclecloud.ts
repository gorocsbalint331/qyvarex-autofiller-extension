...
import * as baseFiller from "./base-filler.ts";
...
class OracleCloud extends BaseFiller {
  ...
  async resolveOracleAddressDependentRules(e) {
    ...
    (C("deps:unfilled", {
      ...
      await (0, baseFiller.waitForComboQuestionsToSettle)(
  ...
  async runComboQuestionAutofillIfNeeded(e, t) {
    ...
    await baseFiller.waitForComboQuestionsToSettle(this.comboQuestionSettleDelayMs);
    let r = await rules.getRules(),
      n = baseFiller.getNewComboQuestionRules(e, r);
    ...
    if (i2.length > 0) {
      ...
      this.answer = baseFiller.mergeComboQuestionAnswer(this.answer, e2, i2);
    }
    return (
