...
async function fillCountryFromCandidates(e, t) {
  ...
  if (isComparableTextMatch(r, t)) return console.info(
    `[Paylocity][Country] already committed currentValue=${jsonPreview(r)} candidateValues=${jsonPreview(t)}`
  ), true;
  let n = e.querySelector("input");
  if (!n) return console.warn(
    `[Paylocity][Country] fill failed reason=missing_search_input currentValue=${jsonPreview(r)} candidateValues=${jsonPreview(t)}`
  ), false;
  let o2 = await ensureCountrySearchAvailable(e, "initial");
  if (!o2) return console.warn(
    `[Paylocity][Country] fill failed reason=search_unavailable phase=initial currentValue=${jsonPreview(r)} candidateValues=${jsonPreview(t)}`
  ), false;
  ...
  if (!i2) return console.warn(
    `[Paylocity][Country] fill failed reason=no_unique_exact_option searchTerm=${jsonPreview(t[0])} candidateValues=${jsonPreview(t)} mountedOptionValues=${jsonPreview(a2)}`
  ...
  for (let r2 = 0; r2 < COUNTRY_COMMIT_POLL_ATTEMPTS; r2++) {
    ...
    if (isComparableTextMatch(readCountryDisplayValue(r3), t)) return console.info(
      `[Paylocity][Country] committed committedValue=${jsonPreview(readCountryDisplayValue(r3))} candidateValues=${jsonPreview(t)}`
    ), true;
    await delay.delay(COUNTRY_POLL_INTERVAL_MS);
  }
  return console.warn(
    `[Paylocity][Country] fill failed reason=commit_readback_mismatch committedValue=${jsonPreview(readCountryDisplayValue(findCountryControl() || e))} candidateValues=${jsonPreview(t)}`
  ), await clearCountrySearch(e), false;
}
...
async function fillCountry(e) {
  ...
  return r ? (console.info(
    `[Paylocity][Country] fill start sourceCountry=${jsonPreview(normalizePaylocityCountry(e))} candidateValues=${jsonPreview(t)} currentValue=${jsonPreview(readCountryDisplayValue(r))}`
  ), await fillCountryWithRollback(r, t)) : (console.warn(
    `[Paylocity][Country] fill failed reason=missing_country_control sourceCountry=${jsonPreview(normalizePaylocityCountry(e))} candidateValues=${jsonPreview(t)}`
  ), false);
}
