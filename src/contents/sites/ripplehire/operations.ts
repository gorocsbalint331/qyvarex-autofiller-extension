...
function preselectExpectedSalaryCurrencyToUsd() {
  ...
  if (!e) return false;
  let t = Array.from(e.options).find((e2) => "$ - usd" === normalizeRipplehireOptionText(f(e2)));
...
function m(e) {
  let t = normalizeRipplehireOptionText(e);
  return !t || t.startsWith("enter ") || t.startsWith("type ") || "select" === t;
}
function h(e, t) {
  let r = t.map(normalizeRipplehireOptionText);
  return e.find((e2) => r.includes(normalizeRipplehireOptionText(e2))) || "";
}
...
function shouldKeepRipplehireCountryValue({ currentCountry: e, country: t, availableValues: r = [] }) {
  if (!e) return false;
  let n = getRipplehireMultiselectCandidateValues({ value: t || "", availableValues: r });
...
function S(e, t, r) {
  ...
  if (n) {
    let e2 = normalizeRipplehireOptionText(t), r2 = Array.from(n.options).find((t2) => normalizeRipplehireOptionText(t2.value) === e2 || normalizeRipplehireOptionText(f(t2)) === e2);
...
function E(e, t, r = t) {
  let n = normalizeRipplehireOptionText(t), o2 = normalizeRipplehireOptionText(r), i2 = e.querySelector(".multiselect-selected-text")?.textContent?.trim(), a2 = normalizeRipplehireOptionText(i2);
  if (a2 === n || a2 === o2) return true;
  let l2 = e.querySelector("select");
  return !!l2 && Array.from(l2.selectedOptions).some((e2) => {
    let t2 = [e2.value, f(e2)].map(normalizeRipplehireOptionText);
...
async function fillMultiselect(e, t) {
  ...
  if (!r) return false;
  let n = getRipplehireMultiselectCandidateValues({ value: t, availableValues: y(e) });
  ...
  let o2 = async (t2) => {
    ...
    for (let e2 of i2) {
      ...
      let i3 = n3.textContent?.replace(/\s+/g, " ").trim() || "", l2 = [i3, o4.value].map(normalizeRipplehireOptionText);
      if (l2.includes(normalizeRipplehireOptionText(t2))) {
...
async function fillCountryFromStateFallback(e) {
  let t = Array.from(document.querySelectorAll(".form-group:not(.hide)")).find((e2) => "country" === normalizeRipplehireOptionText(v(e2)));
  if (!t) return false;
  let r = w(t);
  return !!shouldKeepRipplehireCountryValue({ currentCountry: r, country: e, availableValues: y(t) }) || await fillMultiselect(t, e || "");
}
async function fillInputTextField(e, t, r) {
  let n = e.$input;
  if (!n) return;
  let o2 = n.querySelector("#multi-select");
  if (o2) return await fillMultiselect(n, t);
  if (!t || "" === t.trim()) return;
...
async function uploadResume(e, t, r) {
  let n = await openRipplehireResumeUploadInput();
  if (n) {
    let o2 = U(n);
    ...
    let i2 = await waitForRipplehireResumeFileReady(), s2 = confirmRipplehireResumeUpload();
    ...
    let u2 = await waitForRipplehireResumeParsingComplete();
