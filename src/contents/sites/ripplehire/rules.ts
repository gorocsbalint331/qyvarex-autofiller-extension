...
function extractRules() {
  ...
  for (let r of t) {
    let t2 = getRule(r);
    if (t2) for (let r2 of Array.isArray(t2) ? t2 : [t2]) e.push(...getRipplehirePhoneRules(r2));
  }
  return e;
}
...
function buildExperienceSelectRules(e, t, r, n) {
  ...
  let [i2, ...a2] = e, l2 = a2.map(deriveExperienceSelectSuffix);
...
function getRule(e) {
  ...
  if (m2.length > 0) {
    let e2 = buildExperienceSelectRules(m2, r, n, t);
    if (e2) return e2;
    let i2 = m2[0];
...
function g(e) {
  ...
  if (e.type === enums.FIELD_TYPE.MULTI_SELECT) return getRipplehireCommittedTokenLabels(e.$input);
  let t = e.$input;
...
function getFormSnapshot(e) {
  try {
    return getFormSnapshotFromRules(e || extractRules());
  } catch (e2) {
