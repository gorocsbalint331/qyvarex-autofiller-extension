...
function N(e) {
  ...
  for (let e2 of r) {
    let r2 = e2.querySelector(`ul[jsname="${LISTBOX_UL_JSNAME}"][role="listbox"]`);
    if (!r2) continue;
...
async function ee(e, t, r, n, i2 = true) {
  ...
  let c2 = (e2) => {
      ...
      let n2 = e2.closest('[jsname="wSASue"]'),
        o2 = n2?.querySelector(`ul[jsname="${LISTBOX_UL_JSNAME}"][role="listbox"]`);
      return o2 || N(e2);
    },
...
function er(e, t, r, n) {
  ...
  let p2 = Array.from(e.querySelectorAll(`ul[jsname="${LISTBOX_UL_JSNAME}"][role="listbox"]`)),
    m2 = f2[0];
  if (m2) {
  ...
  for (let e2 = 0; e2 < g2.length; e2++) {
    ...
    if (0 === b3.length) {
      let e3 = n2.parentElement,
        t2 = e3?.querySelector(`ul[jsname="${LISTBOX_UL_JSNAME}"][role="listbox"]`);
      t2 && (b3 = L(t2));
    }
    if (
      (!h3 || 0 === b3.length) &&
  ...
  if (p2) {
    ...
    let l2 = (e3) => {
        ...
        let r3 = e3.closest('[jsname="wSASue"]'),
          n3 = r3?.querySelector(`ul[jsname="${LISTBOX_UL_JSNAME}"][role="listbox"]`);
        return n3 || N(e3);
      },
      s2 = n2.find((e3) => "End Month" === e3.label);
    if (s2?.$input && (!s2.options || 0 === s2.options.length)) {
      let e3 = s2.$input.closest('[jsname="QBGAS"]'),
        r3 = e3?.querySelector(`ul[jsname="${LISTBOX_UL_JSNAME}"][role="listbox"]`);
      if (r3) {
        let e4 = L(r3);
