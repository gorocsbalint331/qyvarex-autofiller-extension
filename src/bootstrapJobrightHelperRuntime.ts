...
async function fetchExtensionAssetText(assetUrl) {
  // Stubbed CSS from the helper bundle is a data: URL. Fetching it on the page
  // can fail under strict CSP (e.g. Greenhouse) — decode without network.
  if (typeof assetUrl === "string" && assetUrl.startsWith("data:")) {
    const comma = assetUrl.indexOf(",")
    if (comma < 0) return ""
    const meta = assetUrl.slice(5, comma)
    const data = assetUrl.slice(comma + 1)
    try {
...
function loadHelperStyleAssets() {
  return (
    ...
    (helperStyleAssetsPromise = Promise.all([
      fetchExtensionAssetText(interCssUrl.default).catch(() => ""),
      fetchExtensionAssetText(globalLessUrl.default).catch(() => ""),
    ]).then(([fontText, rawStyleText]) => {
      let styleText = resolveCssAssetUrls(
        rawStyleText || "",
        typeof globalLessUrl.default === "string" &&
          globalLessUrl.default.startsWith("http")
      ...
      return { fontText: fontText || "", styleText: styleText || "" }
    }))
  )
}
...
async function mountHelperHost() {
  ...
  if (!helperRootElement) {
    // A host rendered by a bundle copy from a reloaded (dead) extension context
    // would otherwise block this copy from ever mounting its UI.
    let staleHost = document.getElementById(HOST_ID)
  ...
  shadowRoot.getElementById(HELPER_ROOT_ID)?.remove()
  let rootElement = document.createElement("div")
  rootElement.id = HELPER_ROOT_ID
  helperRootElement = rootElement
...
function isLinkedInJobPath(pathname) {
  return /^\/(?:jobs|job|easy-apply|in\/[^/]+\/overlay\/apply|hiring|talent)\b/i.test(
    pathname,
  )
}
...
function shouldInitializeProfileOnLoad() {
  ...
  return (
    ...
    (pageUrl.hostname.includes("linkedin.com") &&
      isLinkedInJobPath(pageUrl.pathname))
  )
}
...
let JobrightHelperApp = () => {
  ...
  useEffect(() => {
    // Must not be async: Chrome sends a listener's returned Promise as the reply,
    // which would answer every tab message (e.g. iconClicked) with undefined.
      ...
      if ("urlUpdated" === message?.message) {
        ...
        if (shouldInitializeProfileOnLoad()) {
          refreshProfileState().catch((error) => {
            console.warn("Failed to refresh profile after URL update:", error)
          })
        } else setHelperUiReady(true)
      } else if ("cookieChanged" === message?.message) {
        refreshProfileState({ resetPluginOnError: true })
          .catch((error) => {
            console.warn(
